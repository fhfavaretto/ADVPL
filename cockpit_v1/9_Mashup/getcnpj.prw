...
user function getCNPJ(cTab,cCNPJ)
	...
	if !empty(cTab) .and. len(cCNPJ) == 14
		if isBlind()
			...
			FWMsgRun(,{||cRet:= consulta(cTab,cCNPJ)},'CNPJ.NFE-SEFAZ','Consultando...')
		endif
	endif
	...
	if oCNPJws:consultarCNPJ(cCNPJ)
		...
		elseIf cTab == 'SA2'
			...
			if !empty(oJSON['estabelecimento']['nome_fantasia'])
				oModel:SetValue('SA2MASTER','A2_NREDUZ',Subs(oJSON['estabelecimento']['nome_fantasia'],1,TamSx3("A2_NREDUZ")[1]))
			else
			...
			oModel:SetValue('SA2MASTER','A2_BAIRRO', Subs(oJSON['estabelecimento']['bairro'],1,TamSx3("A2_BAIRRO")[1]))

			oModel:SetValue('SA2MASTER','A2_END',Subs(Alltrim(oJSON['estabelecimento']['logradouro']) + ', ' + Alltrim(oJSON['estabelecimento']['numero']),1,TamSx3("A2_END")[1]))
			...
			next
			//customização SA4 - TRANSPOTRADORAS
		Elseif cTab == 'SA4'

			M->A4_MSBLQL:= if(oJSON['estabelecimento']['situacao_cadastral'] == 'Ativa','2','1')
			If ExistTrigger('A4_MSBLQL')
				RunTrigger(1,Nil,Nil,,'A4_MSBLQL')
			Endif
			...
			if !empty(oJSON['estabelecimento']['pais']['id'])
				CCH->(dbSetOrder(1))
				if CCH->(dbSeek(xFilial('CCH')+ '0' + oJSON['estabelecimento']['pais']['id'] ))
					M->A4_CODPAIS	:=  CCH->CCH_CODIGO
					If ExistTrigger('A4_CODPAIS')
						RunTrigger(1,Nil,Nil,,'A4_CODPAIS')
					Endif
				endif

				SYA->(dbSetOrder(2))
				if SYA->(dbSeek(xFilial('SYA')+ upper(oJSON['estabelecimento']['pais']['nome'])))
					M->A4_PAIS	:= SYA->YA_CODGI
					If ExistTrigger('A4_PAIS')
						RunTrigger(1,Nil,Nil,,'A4_PAIS')
					Endif
				endif
			endif

			M->A4_NREDUZ := oJSON['estabelecimento']['nome_fantasia']

			if empty(M->A4_NREDUZ) //Caso nao possua nome fantasia
				M->A4_NREDUZ := avKey(cRet, 'A1_NREDUZ')
			endif

			If ExistTrigger('A4_NREDUZ')
				RunTrigger(1,Nil,Nil,,'A4_NREDUZ')
			Endif

			M->A4_CEP		:= oJSON['estabelecimento']['cep']
			If ExistTrigger('A4_CEP')
				RunTrigger(1,Nil,Nil,,'A4_CEP')
			Endif

			M->A4_EST		:= oJSON['estabelecimento']['estado']['sigla']
			If ExistTrigger('A4_EST')
				RunTrigger(1,Nil,Nil,,'A4_EST')
			Endif

			M->A4_COD_MUN:= substring(cValToChar(oJSON['estabelecimento']['cidade']['ibge_id']),3,5)
			If ExistTrigger('A4_COD_MUN')
				RunTrigger(1,Nil,Nil,,'A4_COD_MUN')
			Endif

			M->A4_BAIRRO := oJSON['estabelecimento']['bairro']
			If ExistTrigger('A4_BAIRRO')
				RunTrigger(1,Nil,Nil,,'A4_BAIRRO')
			Endif

			M->A4_END    := oJSON['estabelecimento']['logradouro'] + ', ' + oJSON['estabelecimento']['numero']
			If ExistTrigger('A4_END')
				RunTrigger(1,Nil,Nil,,'A4_END')
			Endif

			M->A4_COMPLEM:= oJSON['estabelecimento']['complemento']
			If ExistTrigger('A4_COMPLEM')
				RunTrigger(1,Nil,Nil,,'A4_COMPLEM')
			Endif

			M->A4_DDD		:= oJSON['estabelecimento']['ddd1']
			If ExistTrigger('A4_DDD')
				RunTrigger(1,Nil,Nil,,'A4_DDD')
			Endif

			M->A4_TEL		:= oJSON['estabelecimento']['telefone1']
			If ExistTrigger('A4_TEL')
				RunTrigger(1,Nil,Nil,,'A4_TEL')
			Endif

			M->A4_TELEX		:= oJSON['estabelecimento']['ddd_fax']+oJSON['estabelecimento']['fax']
			If ExistTrigger('A4_TELEX')
				RunTrigger(1,Nil,Nil,,'A4_TELEX')
			Endif

			M->A4_EMAIL	:= oJSON['estabelecimento']['email']
			If ExistTrigger('A4_EMAIL')
				RunTrigger(1,Nil,Nil,,'A4_EMAIL')
			Endif
					/*
					if valType(oJSON['simples']) == 'J'
						M->A1_SIMPNAC:= if(oJSON['simples']['simples'] == 'Sim', '1', '2')
			...
			for nX:=1 to len(oJSON['estabelecimento']['inscricoes_estaduais'])
				if oJSON['estabelecimento']['estado']['id'] == oJSON['estabelecimento']['inscricoes_estaduais'][nX]['estado']['id']
					M->A4_INSEST:= oJSON['estabelecimento']['inscricoes_estaduais'][nX]['inscricao_estadual']
					If ExistTrigger('A4_INSEST')
						RunTrigger(1,Nil,Nil,,'A4_INSEST')
					Endif
					EXIT
				endif
			next
...
If cTab == "SA1"
	cRet := Subs(cRet,1,TamSx3("A1_NOME")[1])	
ElseIf cTab == "SA2"
	cRet := Subs(cRet,1,TamSx3("A2_NOME")[1])	
Else
	cRet := Subs(cRet,1,TamSx3("A4_NOME")[1])	
EndIf	
