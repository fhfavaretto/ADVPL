...
Funùùo que gera TOTVS Report de forma genùrica
@type function
@author Atilio
@since 17/12/2016
@version 1.0
...
User Function zReport()
	...
	//                Titulo             Campo       Picture   Tamanho   Dec Valid         Usado  Tipo F3  Contexto Combo                                                     Ini Padrùo
	...
	//Deixando com espaùos a direita
	...
	DEFINE MSDIALOG oDlgPvt TITLE "zReport - Gerador de TOTVS Report" FROM 000, 000  TO nJanAltu, nJanLarg COLORS 0, 16777215 PIXEL
		//Folders / Pastas
		@ 001, 003 FOLDER oFolderPvt SIZE (nJanLarg/2)-4, (nJanAltu/2)-30 OF oDlgPvt ITEMS	"Relatùrio",;
		...
		@ 001, 001	GROUP oGrpGer TO 055, (nJanLarg/2)-24	PROMPT "Geral: "	OF oScrollRel COLOR RGB(255,0,0), 16777215 PIXEL
			...
			@ 028, 006   SAY   oSayDirec PROMPT "Diretùrio:"     SIZE 030, 007 OF oScrollRel                    PIXEL
			@ 026, 045   MSGET oGetDirec VAR    cGetDirec        SIZE 250, 007 OF oScrollRel COLORS 0, 16777215 VALID (fVldDir()) PIXEL
			@ 043, 006   SAY   oSayTitul PROMPT "Tùtulo:"        SIZE 030, 007 OF oScrollRel                    PIXEL
			@ 041, 045   MSGET oGetTitul VAR    cGetTitul        SIZE 130, 007 OF oScrollRel COLORS 0, 16777215 PIXEL
		
		//Aba do Relatùrio, Grupo Definiùùes
		@ 058, 001	GROUP oGrpDef TO 097, (nJanLarg/2)-24	PROMPT "Definiùùes: "	OF oScrollRel COLOR RGB(255,0,0), 16777215 PIXEL
			@ 070, 006  SAY  oSayOrien  PROMPT "Orientaùùo Padrùo:"     SIZE 060, 007 OF oScrollRel                    PIXEL
		...
		//Aba do Relatùrio, Grupo de Parùmetros
		@ 100, 001	GROUP oGrpPar TO 154, (nJanLarg/2)-24	PROMPT "Parùmetros: "	OF oScrollRel COLOR RGB(255,0,0), 16777215 PIXEL
			...
			@ 127, 006  SAY   oSayPergu  PROMPT "Cùdigo Grupo de Perguntas:"      SIZE 040, 007 OF oScrollRel                    PIXEL
			@ 125, 085  MSGET oGetPergu  VAR    cGetPergu                         SIZE 060, 007 OF oScrollRel COLORS 0, 16777215 PIXEL
			@ 142, 006  SAY   oSayMostr  PROMPT "Mostra pùgina de Parùmetros?"    SIZE 080, 007 OF oScrollRel                    PIXEL
			@ 140, 085  MSCOMBOBOX oCmbMostr VAR cCmbMostr ITEMS aTpPad           SIZE 030, 007 OF oScrollRel COLORS 0, 16777215 PIXEL
			oGetPergu:lActive := .F.
			oCmbMostr:lActive := .F.
		
		//Aba do Relatùrio, Grupo de e-Mails
		...
		@ 065, 001  SAY   oSayQuebr  PROMPT "Quebrar relatùrio por:"      SIZE 060, 007 OF oScrollSQL                    PIXEL
		...
		//Aùùes
		@ (nJanAltu/2)-27, 003	GROUP oGrpAco TO (nJanAltu/2)-1, 	(nJanLarg/2)-1	PROMPT "Aùùes: "	OF oDlgPvt COLOR 0, 16777215 PIXEL
...
/*---------------------------------------------------------------------*
 ...
 | Autor: Daniel Atilio                                                |
 | Data:  17/12/2016                                                   |
 | Desc:  Funùùo que valida o diretùrio digitado                       |
 *---------------------------------------------------------------------*/

Static Function fVldDir()
	Local lRet := .T.
	
	//Se tiver em branco, jù retorna falha
	If Empty(cGetDirec)
		MsgAlert("Preencha um diretùrio")
		lRet := .F.
		
	Else
		//Se o diretùrio nùo existir
		If ! ExistDir(cGetDirec)
			//Se for confirmado tenta criar
			If MsgYesNo("O diretùrio informado nùo existe, deseja criar?", "Atenùùo")
				MakeDir(cGetDirec)
				
				//Se mesmo assim o diretùrio nùo existir, retorna falso
				If ! ExistDir(cGetDirec)
					MsgAlert("Nùo foi possùvel criar o diretùrio.", "Atenùùo")
					lRet := .F.
				EndIf
	...
	If lRet
		...
		//Tratamento para adicionar espaùos em branco
		cGetDirec := PadR(cGetDirec, nLenDirec)
		oGetDirec:Refresh()
	EndIf
Return lRet
...
/*---------------------------------------------------------------------*
 ...
 | Autor: Daniel Atilio                                                |
 | Data:  17/12/2016                                                   |
 | Desc:  Funùùo que valida se utilizarù pergunta / parùmetros         |
 *---------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------*
 ...
 | Autor: Daniel Atilio                                                |
 | Data:  17/12/2016                                                   |
 | Desc:  Funùùo que valida se utilizarù disparo por e-Mail            |
 *---------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------*
 ...
 | Autor: Daniel Atilio                                                |
 | Data:  17/12/2016                                                   |
 | Desc:  Funùùo que valida a query SQL digitada                       |
 *---------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------*
 ...
 | Autor: Daniel Atilio                                                |
 | Data:  17/12/2016                                                   |
 | Desc:  Funùùo que valida o check de ediùùo                          |
 *---------------------------------------------------------------------*/

Static Function fVldChkEdt()
	Local lRet := .T.
	
	//Se tiver checado que irù informar as colunas
	If lChkEdit
		oMsGetCam:lActive := .T.
...
/*---------------------------------------------------------------------*
 ...
 | Autor: Daniel Atilio                                                |
 | Data:  17/12/2016                                                   |
 | Desc:  Funùùo que valida o check de totalizadores                   |
 *---------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------*
 ...
 | Autor: Daniel Atilio                                                |
 | Data:  17/12/2016                                                   |
 | Desc:  Funùùo que que valida a geraùùo do arquivo prw               |
 *---------------------------------------------------------------------*/
...
Static Function fGerar()
	...
	//Se estiver marcado para utilizar parùmetros, porùm a pergunta estiver vazia
	If cCmbUtili == 'S' .And. Empty(cGetPergu)
		MsgAlert("Preencha o cùdigo do grupo de perguntas!", "Atenùùo")
		Return
	EndIf
	
	//Se estiver marcado para enviar email, porùm o destinatùrio estiver vazio
	If cCmbEnvia == 'S' .And. Empty(cGetEmail)
		MsgAlert("Preencha o(s) e-Mail(s) que receberùo o relatùrio!", "Atenùùo")
		Return
	EndIf
	
	//Se nùo tiver consulta sql
	If Empty(cEditSQL)
		MsgAlert("Insira uma consulta SQL!", "Atenùùo")
		Return
	EndIf
	...
	If lChkEdit .And. Len(aDadCel) > 0
		...
		For nAtual := 1 To Len(aDadCel)
			//Se a linha nùo estiver excluùda
		...
		If nOk == 0
			MsgAlert("Nùo existem campos vùlidos para impressùo (verifique campo e/ou tamanho)!", "Atenùùo")
			Return
		EndIf
	EndIf
	
	//Chama a criaùùo do prw
...
/*---------------------------------------------------------------------*
 ...
 | Autor: Daniel Atilio                                                |
 | Data:  17/12/2016                                                   |
 | Desc:  Funùùo que gera o prw com as definiùùes do TOTVS Report      |
 *---------------------------------------------------------------------*/
...
Static Function fCriaPrw()
	...
	//Se o arquivo jù existir
	If File(Alltrim(cGetDirec)+cArquivo)
		//Se deseja sobrepor, exclui o arquivo
		If MsgYesNo("Arquivo jù existe, deseja sobrepor?", "Atenùùo")
	...
	//Senùo gera o conteùdo do arquivo
	Else
		//Cabeùalho
		...
		fWrite(nHdl, 'Relatùrio - ' + cGetTitul + STR_PULA)
		...
		fWrite(nHdl, '	@obs Funùùo gerada pelo zReport()' + STR_PULA)
		fWrite(nHdl, '/*/' + STR_PULA)
		fWrite(nHdl, '	' + STR_PULA)
		...
		If cCmbUtili == 'S'
			fWrite(nHdl, '	' + STR_PULA)
			fWrite(nHdl, '	//Definiùùes da pergunta' + STR_PULA)
			fWrite(nHdl, '	cPerg := "'+cGetPergu+'"' + STR_PULA)
			fWrite(nHdl, '	' + STR_PULA)
			fWrite(nHdl, '	//Se a pergunta nùo existir, zera a variùvel' + STR_PULA)
		...
		If cCmbEnvia == 'S'
			fWrite(nHdl, '	' + STR_PULA)
			fWrite(nHdl, '	//Serù enviado por e-Mail' + STR_PULA)
			fWrite(nHdl, '	lEmail := .T.' + STR_PULA)
		EndIf
		
		fWrite(nHdl, '	' + STR_PULA)
		fWrite(nHdl, '	//Cria as definiùùes do relatùrio' + STR_PULA)
		...
		fWrite(nHdl, '	//Senùo, mostra a tela' + STR_PULA)
		...
		//Definiùùes do relatùrio
		fWrite(nHdl, '/*-------------------------------------------------------------------------------*' + STR_PULA)
		fWrite(nHdl, ' | Func:  fReportDef                                                             |' + STR_PULA)
		fWrite(nHdl, ' | Desc:  Funùùo que monta a definiùùo do relatùrio                              |' + STR_PULA)
		fWrite(nHdl, ' *-------------------------------------------------------------------------------*/' + STR_PULA)
		fWrite(nHdl, '	' + STR_PULA)
		...
		fWrite(nHdl, '	' + STR_PULA)
		fWrite(nHdl, '	//Criaùùo do componente de impressùo' + STR_PULA)
		fWrite(nHdl, '	oReport := TReport():New(	"'+Alltrim(cGetUserF)+'",;		//Nome do Relatùrio' + STR_PULA)
		fWrite(nHdl, '								"'+Alltrim(cGetTitul)+'",;		//Tùtulo' + STR_PULA)
		fWrite(nHdl, '								cPerg,;		//Pergunte ... Se eu defino a pergunta aqui, serù impresso uma pùgina com os parùmetros, conforme privilùgio 101' + STR_PULA)
		fWrite(nHdl, '								{|oReport| fRepPrint(oReport)},;		//Bloco de cùdigo que serù executado na confirmaùùo da impressùo' + STR_PULA)
		fWrite(nHdl, '								)		//Descriùùo' + STR_PULA)
		...
		EndIf
		fWrite(nHdl, '	' + STR_PULA)
		fWrite(nHdl, '	//Criando a seùùo de dados' + STR_PULA)
		fWrite(nHdl, '	oSectDad := TRSection():New(	oReport,;		//Objeto TReport que a seùùo pertence' + STR_PULA)
		fWrite(nHdl, '									"Dados",;		//Descriùùo da seùùo' + STR_PULA)
		fWrite(nHdl, '									{"QRY_AUX"})		//Tabelas utilizadas, a primeira serù considerada como principal da seùùo' + STR_PULA)
		fWrite(nHdl, '	oSectDad:SetTotalInLine(.F.)  //Define se os totalizadores serùo impressos em linha ou coluna. .F.=Coluna; .T.=Linha' + STR_PULA)
		fWrite(nHdl, '	' + STR_PULA)
		fWrite(nHdl, '	//Colunas do relatùrio' + STR_PULA)
		
		//Se foi informado as colunas manualmente
		If lChkEdit .And. Len(aDadCel) > 0
			//Percorre os dados informados pelo usuùrio
			For nAtual := 1 To Len(aDadCel)
				//Se a linha nùo estiver excluùda
				If ! aDadCel[nAtual][Len(aHeaderCam) + 1]
		...
		//Senùo, pega todas da estrutura da query
		Else
			DbSelectArea('SX3')
		...
		//Impressùo do relatùrio
		fWrite(nHdl, '/*-------------------------------------------------------------------------------*' + STR_PULA)
		fWrite(nHdl, ' | Func:  fRepPrint                                                              |' + STR_PULA)
		fWrite(nHdl, ' | Desc:  Funùùo que imprime o relatùrio                                         |' + STR_PULA)
		fWrite(nHdl, ' *-------------------------------------------------------------------------------*/' + STR_PULA)
		fWrite(nHdl, '	' + STR_PULA)
		...
		fWrite(nHdl, '	' + STR_PULA)
		fWrite(nHdl, '	//Pegando as seùùes do relatùrio' + STR_PULA)
		...
		fWrite(nHdl, '	//Executando consulta e setando o total da rùgua' + STR_PULA)
		...
		fWrite(nHdl, '		//Incrementando a rùgua' + STR_PULA)
		...
		If File(Alltrim(cGetDirec)+cArquivo)
			If MsgYesNo("Arquivo gerado, deseja visualizar?", "Atenùùo")
