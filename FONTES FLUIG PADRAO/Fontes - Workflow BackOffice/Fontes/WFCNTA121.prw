...
oModel := MPFormModel():New('WFCNT121', /*bPreValidacao*/, {|oModel,oModelCT|U_CNFlgVldSt(oModel,oModelCT)}/*bPosValidacao*/, {|oModel|A121LibDoc(oModel)}, /*bCancel*/ )
...
Static Function WF121Model(aCampos,cStru,oStru)
Local nCampo := 1
Local cCampo := ''
...
Static Function WF121View(aCampos,cStru,oStru)
Local nCampo 		:= 0
Local cCampo 		:= ''
Local lAltCampo 	:= .T.
Local aModelFlg	:= {}
...
While (cAliasTemp)->(!Eof())
	...
	AADD(aAux, (cAliasTemp)->CNE_CC     + U_Wf300Descr("CNB_CC"    ,(cAliasTemp)->CNE_CC)	 	)
	AADD(aAux, (cAliasTemp)->CNE_CONTA  + U_Wf300Descr("CNB_CONTA" ,(cAliasTemp)->CNE_CONTA) 	)
	AADD(aAux, (cAliasTemp)->CNE_ITEMCT + U_Wf300Descr("CNB_ITEMCT",(cAliasTemp)->CNE_ITEMCT)	)
	AADD(aAux, (cAliasTemp)->CNE_CLVL   + U_Wf300Descr("CNB_CLVL"  ,(cAliasTemp)->CNE_CLVL)	)
...
/*/
//-------------------------------------------------------------------
Static Function A121LibDoc(oModel)
...
lRet := U_MTFlgLbDoc(cNum,cUser,cAprov,cTipo,cFluig,cParecer,oModelCT)
